...
> 若 `知识图谱.html` 不存在（如手动删除了），可重新生成（`$PYTHON` 指可用 Python：优先 `~/.workbuddy/binaries/python/envs/default/Scripts/python.exe`，否则系统 `python`/`python3`）。`{{BUILDER_TOOLS_DIR}}` 由生成方（ontology-app-builder）在填充本模板时替换为其实际安装的 `tools/` 目录绝对路径：
```
$PYTHON ^
  {{BUILDER_TOOLS_DIR}}/build_knowledge_graph.py ^
  ont_yaml/ ^
  knowledge-graph-data.json
$PYTHON ^
  {{BUILDER_TOOLS_DIR}}/build_graph_html.py ^
  knowledge-graph-data.json ^
  {{BUILDER_TOOLS_DIR}}/echarts.min.js ^
  知识图谱.html ^
  "{{DOMAIN_NAME}}·本体知识图谱"
```
