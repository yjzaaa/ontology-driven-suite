...
- 技能包根目录（下称 `$SKILL_DIR`）：即本 SKILL.md 所在目录。本技能为**项目级安装**（`<项目根>/.agents/skills/ontology-app-builder/`），下文 engine/tools/specs 路径均相对 `$SKILL_DIR`，执行命令前先将其展开为实际绝对路径。
- 运行引擎（engine）路径：`$SKILL_DIR/engine/`，生成领域技能时整体拷贝。
- 图谱工具（tools）路径：`$SKILL_DIR/tools/`（`build_knowledge_graph.py` + `build_graph_html.py` + `knowledge-graph-template.html`），第 2 段完成后自动运行生成知识图谱。
...
```
$PYTHON \
  $SKILL_DIR/tools/build_knowledge_graph.py \
  <第2段yaml输出目录> \
...
```
$PYTHON \
  $SKILL_DIR/tools/build_graph_html.py \
  <第2段yaml输出目录>/../knowledge-graph-data.json \
  $SKILL_DIR/tools/echarts.min.js \
...
2. 拷贝引擎：把 `$SKILL_DIR/engine/` 整个目录复制到 `<domain-slug>/engine/`。
